...
<div class = 'question'>
	Which integer-coefficient polynomials $P$ have the property that $P(n)$ is a perfect power for every integer $n$? 
</div>
...
<div class='definition'>
	We say an integer $n$ is a <em>perfect power</em> if $n = m^k$ for integers $m$ and $k$ with $k > 1$. 
</div>

<div class='definition'>
	We say a polynomial $P \in \mathbb{Z}[x]$ is a <em>perfect power</em> if $P(x) = Q(x)^k$ for some polynomial $Q(x) \in \mathbb{Z}[x]$ and some integer $k > 1$. 
</div>

If a polynomial $P$ is a perfect power, then $P(n)$ is a perfect power for every integer $n$ &mdash; we can write $P(n)$ as $Q(n)^k$. So it's natural to ask whether the converse is true &mdash; if we know that $P(n)$ is a perfect power for every integer $n$, then must $P$ *itself* be a perfect power? It turns out the answer is yes. 

<div class='theorem'>
	If a polynomial $P \in \mathbb{Z}[x]$ has the property that $P(n)$ is a perfect power for all integers $n$, then $P$ itself must be a perfect power. 
</div>
...
<div class='theorem' text='Schur&apos;s Theorem'>
	If $P \in \mathbb{Z}[x]$ is a nonconstant polynomial, then there are infinitely many primes which divide $P(n)$ for some integer $n$. 
</div>

<div class='proof'>
	Assume for contradiction that the only primes dividing $P(n)$, over all $n$, are $p_1$, $p_2$, $\ldots$, $p_r$. Now the idea is to construct a bunch of values of $n$ which have the same value of $P(n)$, by imposing conditions mod powers of these primes. 
	<br/><br/>
	...
	<br/><br/>
	So then there is some value which $P$ takes infinitely many times &mdash; namely, either $P(a)$ or $-P(a)$ &mdash; which means $P$ must be constant. 
</div>
